...
def scrape_hotels(url):
    ...
    hotel_rating = driver.execute_script(
        "return document.getElementsByClassName(\"b5cd09854e d10a6220b4\")")

    hotel_price = driver.execute_script(
        "return document.getElementsByClassName(\"fcab3ed991 bd73d13072\")")

    # will have to every other values
    hotel_distance = driver.execute_script(
        "return document.getElementsByClassName(\"f4bd0794db b4273d69aa\")")

    hotel_location = driver.execute_script(
        "return document.getElementsByClassName(\"fc63351294 a168c6f285 e0e11a8307 a25b1d9e47\")")

    hotel_image = driver.execute_script(
        "return document.getElementsByClassName(\"b8b0793b0e\")")
...
def main():
    ...
    #stopped at index 129 Mckiney Texas resume there
    for city in city_state:
        
        data = city.split(",")
        #remove space the state
        data[1] = data[1].replace(" ", "")

        location = gen_location(data[0], data[1], "United States of America")
        check_in = "2022/4/23"
        check_out = "2022/4/24"
        adults = 1
        children = 0
        rooms = 1
        urls = generate_url(location, check_in, check_out, adults, children, rooms)
        dump = scrape_hotels(urls)
        database_insert(dump, city)
